...
//         };
//         setOrderItems([item]);
//         setTotal(product.price * productQuantity);
//       } else {
...
//           }));
//           setOrderItems(items);
//           setTotal(parseFloat(cartData.total_price));
//         } else {
...
//         // Notes (optional)
//         notes: `Order placed via ${productId ? 'Buy Now' : 'Cart'} checkout`,
...
//         setMessage({ 
//           type: 'success', 
//           text: `Order ${result.order_id} placed successfully!` 
//         });
        
//         // Redirect to shop after 3 seconds (or create a success page)
//         setTimeout(() => {
//           router.push(`/Pages/shop?order_success=${result.order_id}`);
//         }, 3000);
...
//           errorMessage = `Product availability issue: ${errorData.items[0]}`;
//         } else if (errorData.customer_info) {
//           errorMessage = 'Please check your customer information.';
//         } else if (errorData.payment_info) {
...
//               <CreditCard size={18} className="mr-2 text-[#F9B651]" />
//               Payment Details
//             </h3>
    ...
    try {
      ...
      if (response.ok) {
        ...
        const item = {
          ...
          quantity: productQuantity,
          image: product.main_image,
          weight: product.weight || 0.5 // Default weight if not available
        };
        setOrderItems([item]);
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      const orderData = {
        ...
        // Shipping method - FIXED: Use proper field name
        shipping_method: selectedShippingOption.name, // Use display name
        ...
        // Pricing breakdown - FIXED: Ensure proper decimal format
        subtotal: parseFloat(subtotal.toFixed(2)),
        tax_amount: parseFloat(tax.toFixed(2)),
        total_amount: parseFloat(total.toFixed(2)),
        
        // FIXED: Add delivery instructions properly
        delivery_instructions: formData.delivery_instructions || '',
        
        // FIXED: Add estimated delivery
      ...
      if (response.ok && responseData.success) {
        ...
        // Handle specific error messages - FIXED: Better error handling
        let errorMessage = 'Failed to place order. Please try again.';
